local MovingGoalsEasy, parent = torch.class('MovingGoalsEasy', 'MazeBase')

math = require ('math')

function MovingGoalsEasy:__init(opts, vocab)
    opts.height = 4
    opts.width = 4
    ...
    if self.agent == nil then
        ...
        for i = 1, self.nagents do
            self.agents[i] =  self:place_item({type = 'agent'}, 1, 4)
        end
        self.agent = self.agents[1]
    end
end

function MovingGoalsEasy:update()
    parent.update(self)
...
function MovingGoalsEasy:get_reward()
