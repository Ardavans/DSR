...
function setup(_opt)
    ...
    local framework = require(opt.framework)--dofile('../mbwrap/init.lua')

    assert(framework)
    local gameEnv, gameActions
    if opt.framework == 'alewrap' then
        gameEnv = framework.GameEnvironment(opt)
    else
        
        if opt.framework == '../games/init.lua' then
            gameEnv = framework.GameEnvironment(_opt.seed+100) --pass port
        else --mazebase
            gameEnv = framework.GameEnvironment(_opt.game_name)
        end
    end
    ...
    _opt.agent_params.game_name = _opt.game_name
    ...
    local agent = dsr[_opt.agent](_opt.agent_params)
